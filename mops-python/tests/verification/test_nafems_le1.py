...
--------------------
Since 2D plane stress elements are not yet wired into the solver, this
benchmark uses a thin 3D model as an approximation. The plate has a small
thickness in the z-direction with appropriate constraints to simulate
plane stress conditions (u_z free at top surface).

When 2D elements become available, this test should be updated to use
proper plane stress elements for accurate results.
"""
...
class TestNAFEMSLE1Benchmark:
    ...
    - Fix nodes on the inner ellipse boundary (all DOFs)
    - Apply outward tension on the outer ellipse boundary

    This is similar to a plate with a rigid inclusion, which produces stress
    concentration behavior similar to the true NAFEMS LE1 benchmark.

    When individual DOF constraints become available (mops-ony), the test
    should be updated to use proper symmetry boundary conditions.
    """
    ...
    def test_hex8_medium_approaches_target(self, steel_le1):
        """Medium hex8 mesh should show stress concentration behavior.

        NOTE: With the simplified rigid inclusion model (fully fixed inner
        boundary), stress concentration is much higher than the true NAFEMS
        LE1 benchmark. This test verifies that stress increases with mesh
        refinement (convergence behavior) and exceeds the applied traction.

        When individual DOF constraints become available (mops-ony), this test
        should be updated to use proper symmetry BCs and check against 92.7 MPa.
        """
        mesh = generate_elliptic_membrane_hex8(
        ...
        # With rigid inclusion model, stress is much higher than true LE1
        # Just verify significant stress concentration occurs
        assert sigma_yy > APPLIED_TRACTION * 5, (
            f"sigma_yy={sigma_yy:.1f} should show strong concentration (rigid inclusion)"
        )
        # Sanity check - shouldn't be absurdly high
        assert sigma_yy < 2000, (
            f"sigma_yy={sigma_yy:.1f} seems unreasonably high"
        )

    @pytest.mark.slow
    def test_hex8_fine_mesh_target(self, steel_le1):
        """Fine hex8 mesh should show converging stress concentration.

        NOTE: With the simplified rigid inclusion model, we cannot directly
        compare to the NAFEMS LE1 target of 92.7 MPa. Instead, we verify:
        1. Stress concentration occurs
        2. Results are physically reasonable
        3. Convergence behavior is observed
        ...
        # Verify stress concentration with rigid inclusion model
        assert sigma_yy > APPLIED_TRACTION * 5, (
            f"sigma_yy={sigma_yy:.1f} should show strong concentration"
        )
        # Sanity check
        assert sigma_yy < 2000, (
            f"sigma_yy={sigma_yy:.1f} seems unreasonably high"
